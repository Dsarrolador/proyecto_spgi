    ...
    <div class="spgi-card">
      ...
      <div class="card-body-spgi">
        <div class="table-responsive">
          <table class="table table-spgi table-bordered align-middle mb-0">
            <thead>
              <tr>
            ...
            @forelse($documents as $doc)
              <tr>
                ...
                <td class="text-center">
                  <div class="acciones">
                    @if($doc->estado == 'Validado')
                    <a href="{{ route('wiki.download', $doc->id) }}" class="btn btn-success text-white" style="width: 36px; height: 36px; padding: 0;" title="Descargar">
                      <i class="bi bi-download"></i>
                    </a>
                    @else
                    <button class="btn btn-secondary text-white-50" style="width: 36px; height: 36px; padding: 0;" title="No disponible hasta validar" disabled>
                      <i class="bi bi-download"></i>
                    </button>
                    @endif

                    <button class="btn btn-warning" type="button" data-bs-toggle="modal" data-bs-target="#modalEditar{{ $doc->id }}" style="width: 36px; height: 36px; padding: 0;" title="Editar">
                      <i class="bi bi-pencil-square"></i>
                    </button>

                    <button class="btn btn-danger" type="button" data-bs-toggle="modal" data-bs-target="#confirmarEliminar{{ $doc->id }}" style="width: 36px; height: 36px; padding: 0;" title="Eliminar">
                      <i class="bi bi-trash"></i>
                    </button>

                    @if(auth()->user()->role && in_array(auth()->user()->role->nombre, ['Administracion', 'Encargado']) && $doc->estado !== 'Validado')
                    <button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#confirmarAprobar{{ $doc->id }}" style="width: 36px; height: 36px; padding: 0;" title="Validar">
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width: 400px;">
      <div class="modal-content border-0 shadow" style="border-radius: 20px;">
        <div class="modal-body text-center py-4 px-3">
          ...
          <p class="text-danger small mb-0"><i class="bi bi-exclamation-triangle me-1"></i> Se borrará físicamente del servidor.</p>
          
          <div class="d-flex justify-content-center gap-2 mt-4 w-100">
            <button type="button" class="btn btn-light border px-3 w-50" style="border-radius: 12px; font-weight: 600; font-size: .9rem;" data-bs-dismiss="modal">Cancelar</button>
            <form action="{{ route('wiki.destroy', $doc->id) }}" method="POST" class="m-0 w-50">
              @csrf
              @method('DELETE')
              <button type="submit" class="btn btn-danger w-100 px-3" style="border-radius: 12px; font-weight: 600; font-size: .9rem; background: linear-gradient(135deg, #e63946, #d00000); border:0;">Eliminar</button>
            </form>
          </div>
        </div>
      </div>
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width: 400px;">
      <div class="modal-content border-0 shadow" style="border-radius: 20px;">
        <div class="modal-body text-center py-4 px-3">
          ...
          <p class="text-secondary small mb-0">¿Estás seguro de que deseas validar <strong>{{ $doc->title }}</strong>?<br>Esta acción cambiará el estado a <span class="badge bg-success">Validado</span>.</p>
          
          <div class="d-flex justify-content-center gap-2 mt-4 w-100">
            <button type="button" class="btn btn-light border px-3 w-50" style="border-radius: 12px; font-weight: 600; font-size: .9rem;" data-bs-dismiss="modal">Cancelar</button>
            <form action="{{ route('wiki.approve', $doc->id) }}" method="POST" class="m-0 w-50">
              @csrf
              <button type="submit" class="btn btn-primary w-100 px-3" style="border-radius: 12px; font-weight: 600; font-size: .9rem; background: linear-gradient(135deg, var(--spgi-primary), var(--spgi-primary-2)); border:0;">Aprobar</button>
            </form>
          </div>
        </div>
      </div>
        ...
        <form action="{{ route('wiki.update', $doc->id) }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <small class="text-muted">Presiona **Espacio** o **Coma** para fijar la etiqueta.</small>
            </div>

            <div class="mb-3">
              <label class="form-label fw-semibold">Cambiar Archivo (Opcional)</label>
              <input type="file" class="form-control" name="file">
              <small class="text-muted">Tamaño máximo 30MB. Dejar en blanco para mantener el actual.</small>
            </div>
          </div>
    ...
    <div class="modal-content">
      ...
      <form action="{{ route('wiki.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <div class="mb-3">
            <label class="form-label fw-semibold">Título <span class="text-danger">*</span></label>
            <input type="text" class="form-control" name="title" required placeholder="Ej: Manual de Usuario 2026">
          </div>

          <div class="mb-3">
            <label class="form-label fw-semibold">Descripción</label>
            <textarea class="form-control" name="description" rows="3" placeholder="Información adicional sobre el documento..."></textarea>
          </div>
          ...
          <div class="mb-3">
            ...
            <small class="text-muted">Tamaño máximo 30MB.</small>
          </div>
        </div>
