...
@section('page_title', 'Editar Requerimiento Administrativo')
        ...
        <div class="col-md-8">
            <div class="card border-0 shadow-sm rounded-4 bg-surface-glass" style="border: 1px solid var(--border-main) !important; backdrop-filter: blur(20px);">
                <div class="card-header bg-transparent border-0 pt-4 px-4 pb-2">
                    <h3 class="fw-bold mb-0 text-gradient"><i class="bi bi-pencil-square me-2 text-warning"></i>Editar Requerimiento</h3>
                    <p class="text-muted small mb-0">Modifica los detalles del requerimiento administrativo seleccionado.</p>
                </div>
                
                <div class="card-body p-4">
                    <form action="{{ route('requerimientos-administrativos.update', $requerimiento->id) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <div class="mb-3">
                            <label for="titulo" class="form-label fw-bold small text-muted">Título / Asunto</label>
                            <input type="text" name="titulo" id="titulo" class="form-control" value="{{ $requerimiento->titulo }}" required>
                        </div>

                        <div class="mb-3">
                            <label for="descripcion" class="form-label fw-bold small text-muted">Descripción detallada</label>
                            <textarea name="descripcion" id="descripcion" rows="4" class="form-control">{{ $requerimiento->descripcion }}</textarea>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="estado" class="form-label fw-bold small text-muted">Estado</label>
                                <select name="estado" id="estado" class="form-select">
                                    ...
                                    <option value="Cancelado" {{ $requerimiento->estado == 'Cancelado' ? 'selected' : '' }}>Cancelado</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="asignado_user_id" id="asignado_user_id" class="form-select">
                                    ...
                                    @foreach($usuarios as $u)
                                        <option value="{{ $u->id }}" {{ $requerimiento->asignado_user_id == $u->id ? 'selected' : '' }}>{{ $u->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="fecha_limite" id="fecha_limite" class="form-control" value="{{ $requerimiento->fecha_limite ? $requerimiento->fecha_limite->format('Y-m-d') : '' }}">
                            </div>
                        </div>
                        ...
                        <div class="card border-0 mb-3" style="border-radius: 12px; background: rgba(255,255,255,0.03); border: 1px solid var(--border-main) !important;">
                            <div class="card-body">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="es_recurrente" id="es_recurrente" value="1" {{ $requerimiento->es_recurrente ? 'checked' : '' }}>
                                ...
                                <div id="freq_container" class="mt-3 {{ $requerimiento->es_recurrente ? '' : 'd-none' }}">
                                    <div class="mb-3">
                                        ...
                                        <select name="frecuencia" id="frecuencia" class="form-select">
                                            <option value="Diario" {{ $requerimiento->frecuencia == 'Diario' ? 'selected' : '' }}>Diario</option>
                                            <option value="Semanal" {{ $requerimiento->frecuencia == 'Semanal' ? 'selected' : '' }}>Semanal</option>
                                            <option value="Quincenal" {{ $requerimiento->frecuencia == 'Quincenal' ? 'selected' : '' }}>Quincenal</option>
                                            <option value="Mensual" {{ $requerimiento->frecuencia == 'Mensual' || !$requerimiento->frecuencia ? 'selected' : '' }}>Mensual</option>
                                            <option value="Semestral" {{ $requerimiento->frecuencia == 'Semestral' ? 'selected' : '' }}>Semestral</option>
                                            <option value="Anual" {{ $requerimiento->frecuencia == 'Anual' ? 'selected' : '' }}>Anual</option>
                                        </select>
                                    </div>

                                    <div class="mb-3">
                                        <label for="fecha_inicio_recurrencia" class="form-label small fw-bold text-muted">Fecha de Inicio de Recurrencia</label>
                                        <input type="date" name="fecha_inicio_recurrencia" id="fecha_inicio_recurrencia" class="form-control" value="{{ $requerimiento->fecha_inicio_recurrencia ? $requerimiento->fecha_inicio_recurrencia->format('Y-m-d') : now()->format('Y-m-d') }}">
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            ...
                            <button type="submit" class="btn btn-warning rounded-pill px-4 text-dark fw-bold">Guardar Cambios</button>
                        </div>
                    </form>
                </div>
            </div>
